...
void main() {
  group('AddProductScreen Widget Tests', () {
    ...
    testWidgets(
      ...
      // Fill required fields so validation passes up to the images check
      // TextFormFields order: Product Name, Description, Price
      await tester.enterText(find.byType(TextFormField).at(0), 'Test Product');
      await tester.enterText(find.byType(TextFormField).at(1), 'A valid description for testing');
      await tester.enterText(find.byType(TextFormField).at(2), '9.99');
      await tester.pumpAndSettle();

      // Find and tap the Create Product button (ensure visible first)
      final createButton = find.widgetWithText(ElevatedButton, 'Create Product');
