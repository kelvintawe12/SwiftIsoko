...
class _HomeScreenState extends ConsumerState<HomeScreen> {
    ...
    return Scaffold(
            ...
            child: currentUserAsync.when(
              data: (u) => CircleAvatar(
                ...
                child: u?.profileImageUrl == null
                    ? Text(u != null ? Formatters.getInitials(u.name) : 'U',
      ...
      body: productsAsync.when(
        data: (productList) {
          ...
          final allPrices =
              productList.map((p) => p.price).where((p) => p > 0).toList();
          final maxFound = allPrices.isNotEmpty
              ? allPrices.reduce((a, b) => a > b ? a : b)
              : 2000.0;
          final maxPossible =
              (maxFound <= 0 ? 2000.0 : maxFound).ceilToDouble();
                        ...
                        return Stack(
                          children: [
                            ...
                            Positioned(
                              ...
                              child: Column(
                                children:
                                    List.generate(_bannerSlides.length, (i) {
                      ...
                      child: Row(mainAxisSize: MainAxisSize.min, children: [
                        IconButton(
                            onPressed: () => _showFilterSheet(
                                context, productList, maxPossible),
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return _SmallCard(
                        ...
                        onSaveToggle: () async {
                          ...
                          final notifier = ref
                              .read(savedIdsNotifierProvider(userId).notifier);
  ...
  void _showFilterSheet(BuildContext context, List<ProductModel> productList,
      double maxPossible) {
    showModalBottomSheet(
        ...
        builder: (ctx) {
          return StatefulBuilder(builder: (context, setStateSheet) {
            ...
            RangeValues current =
                RangeValues(_minPrice ?? minPossible, _maxPrice ?? maxPossible);
            if (current.start < minPossible)
              current = RangeValues(minPossible, current.end);
            if (current.end > maxPossible)
              current = RangeValues(current.start, maxPossible);
...
class _SmallCard extends ConsumerWidget {
  ...
  const _SmallCard(
      {required this.product, required this.onTap, this.onSaveToggle});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final userId = ref.watch(currentUserIdProvider);
    final savedAsync = userId != null
        ? ref.watch(savedIdsNotifierProvider(userId))
