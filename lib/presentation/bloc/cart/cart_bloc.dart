...
class CartBloc extends Bloc<CartEvent, CartState> {
  ...
  void _onAddToCart(AddToCart event, Emitter<CartState> emit) {
    final currentState =
        state is CartLoaded ? state as CartLoaded : const CartLoaded(items: []);
    ...
    final updated = raw.map<CartItem>((e) {
      ...
      return CartItem.fromListing(e as Listing, quantity: 1);
    }).toList();
    ...
    if (idx >= 0) {
      ...
      updated.add(CartItem.fromListing(event.listing, quantity: 1));
    }
  ...
  void _onRemoveFromCart(RemoveFromCart event, Emitter<CartState> emit) {
    ...
    final normalized = raw2
        .map<CartItem>((e) =>
            e is CartItem ? e : CartItem.fromListing(e as Listing, quantity: 1))
        .toList();
    final updated =
        normalized.where((ci) => ci.listing.id != event.listing.id).toList();
    emit(CartLoaded(items: updated));
  }
